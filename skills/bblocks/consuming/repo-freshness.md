...
- The check is read-only up to step 5. Only an approved or pre-authorised fast-forward changes
  anything.
- When consuming, a local clone of a register is just a cache. Prefer fetching the published
  `register.json` and block outputs over HTTP. Use a clone only after it passes this check.
