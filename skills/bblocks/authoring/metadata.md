...
| `license` | `{ name?, url? }` object (at least one required), or a plain string as shorthand for `{ name: <string> }`. Overrides the register-wide `license` from `bblocks-config.yaml` if set — see [register-config.md](register-config.md). |
...
| Field | Description |
|-------|-------------|
| `schema` | URL to the JSON Schema. Auto-derived when `schema.yaml` or `schema.json` is present, or can be set directly to a bare external URL — either way the postprocessor fetches and annotates it the same way. |
