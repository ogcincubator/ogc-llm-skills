...
### Inform the user about GitHub Pages

**Tell the user:** GitHub Pages must be enabled for the public-facing outputs to work — the HTML
documentation, `register.json`, and per-block `bblock.json` index files are all generated by the CI
workflow and served from there. Without it the register exists on GitHub but isn't publicly accessible.

They need to go to **Settings → Pages → Source** and select **GitHub Actions** (not a branch).
